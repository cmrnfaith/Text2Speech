...
{
  const byte *soundData = &SoundData[soundNum * 0x400];
  while (count-- > 0) {
    byte s = pgm_read_byte(&soundData[soundPos & 0x3ffu]);
    sound((byte) (s & volume));
    pause(pitch1);
...
void TTS::sayPhonemes(const char *textp)
{
  ...
  if (phonemesToData(textp, s_phonemes)) {
    ...
    for (phonemeIn = 0, modifierIn = 0; phonemes[phonemeIn];
      ...
      sound1Duration = 0x800;	// play half of sound 1
      if (sound2Num == sound1Num)
        byte2 = duration;

      // S11
      if ((byte2 >> 1) == 0) {
        sound1Duration = 0xfff;	// play all of sound 1
      } else {
        // The fade speed between the two sounds
        fadeSpeed = (sound1Duration + (byte2 >> 1)) / byte2;

        if (duration == 1) {
          sound2Stop = 0x400;	// dont play sound2
          sound1Duration = 0xfff;	// play all of sound 1
          pitch1 = 12;
        }
      }
      ...
      SoundPos = 0;
      do {
        byte sound1Stop = (sound1Duration >> 2) & 0x3ffu;
        ...
        // s23
        if (sound1Duration != 0xfff && duration < byte2) {
          // Fade sound1 out
